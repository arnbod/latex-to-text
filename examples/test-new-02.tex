...
\begin{itemize}
  ...
  \item \textbf{Puissance.}\index{puissance}\index{**@€260€} \ci{3**2} pour $3^2=9$ ; puissance négative \ci{10**-3} pour $10^{-3} = 0.001$.
...
À partir de maintenant, il est préférable que tu travailles dans un éditeur de texte dédié à \Python. Tu dois alors explicitement demander d'afficher\index{print@€271€} le résultat :
\begin{lstlisting}
print(2+2)
...
La commande \og{}\ci{a = 3}\fg{}\index{=@€274€}\index{affectation} signifie que j'ai une variable \og{}\ci{a}\fg{} associée à la valeur $3$. 
...
\end{center} 	 
 ...
 % Autre exemple : \ci{float()} transforme une chaîne de caractères en un nombre : \ci{float("+1.234567")} renvoie le nombre flottant \ci{1.234567}.
  ...
  \item \textbf{Le module \ci{math}.}
	...
	Toutes les fonctions ne sont pas directement accessibles. Elles sont souvent regroupées dans des \defi{modules}\index{module}. Par exemple le module \ci{math}\index{math@€303€}\index{module!math@€304€} contient les fonctions mathématiques. Tu y trouves par exemple la fonction racine carrée \ci{sqrt()}\index{sqrt@€306€}\index{racine carree@racine carrée} (\emph{square root}). Voici comment l'utiliser : 
\begin{center}
\begin{minipage}{0.5\textwidth}
\begin{lstlisting} 
from math import *
...
La première ligne importe toutes les fonctions du module \ci{math}\index{import@€308€}, la seconde calcule $x = \sqrt{2}$ (en valeur approchée) et ensuite on affiche $x$ et $x^2$.	
  \item \textbf{Sinus et cosinus.} 
  
  Le module \ci{math} contient les fonctions trigonométriques sinus\index{sin@€310€} et cosinus\index{cos@€311€} et même la constante \ci{pi}\index{pi@€313€}\index{€58€} qui est une valeur approchée de $\pi$. Attention, les angles sont exprimés en radians.
...
Dans le module \ci{math} il y aussi des fonctions pour arrondir un nombre décimal : 
  \begin{itemize}
    \item \ci{round()}\index{round@€316€} arrondit à l'entier le plus proche : \ci{round(5.6)} renvoie \ci{6}, \ci{round(1.5)} renvoie \ci{2}.
    \item \ci{floor()}\index{floor@€322€} renvoie l'entier inférieur ou égal : \ci{floor(5.6)} renvoie \ci{5}.
    \item \ci{ceil()}\index{ceil@€326€} renvoie l'entier supérieur ou égal : \ci{ceil(5.6)} renvoie \ci{6}.
  \end{itemize}       
...
\begin{enumerate}
  \item  La fonction \Python{} pour le pgcd est \ci{gcd(a,b)}\index{gcd@€331€}\index{pgcd} (sans le \og{}p\fg{}, pour \emph{greatest common divisor}). Calcule le pgcd de $a = 10\,403$ et $b = 10\,506$. Déduis-en le ppcm\index{ppcm} de $a$ et $b$. La fonction ppcm n'existe pas, tu dois utiliser la formule :
  ...
  \emph{Indication.} \ci{abs()}\index{abs@€341€}\index{valeur absolue} désigne la fonction valeur absolue.
